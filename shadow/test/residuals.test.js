/* Falsifiability test for Shadow's residual gaps: the unfurl tags, the
 * source-list links, the generic (non-Meta, non-Google) upload path, the
 * negative-dividend caption, page width at 400px, and published-data drift.
 *
 * Serves the whole website/ tree from a local static HTTP server (Shadow
 * lives at /shadow/ under the rest of the site) and drives the real page
 * through Playwright (headless Chromium), checking five things plus a
 * sixth, filesystem-only check:
 *
 ...
 *      Note: this fixture is deliberately not Google-Takeout-shaped, since
 *      Google Takeout gets its own shape-based parser (parsers/google.js);
 *      it exercises the fallback path instead — see
 *      fixtures/generic/fixture-data.js.
 ...
 *   6. Each website/shadow/data/*.json file is byte-identical to the
...
// website/shadow/data/*.json are publish-safe copies of files kept in this
// project's private working directory (not itself under version control),
// produced by a publish-safe transform script. This asserts each published
// file is byte-identical to that transform's output of the current
// private-directory source, so the suite fails the moment the two diverge —
// the regression test for an earlier defect where the two silently went
// out of sync. If that private directory is absent (a fresh clone that only
...
async function main() {
  ...
  try {
    ...
    // og:image (and twitter:image) must be absolute — the Open Graph spec
    // requires it, and some unfurlers silently drop a relative one. Shadow
    // has a settled URL, so there is no reason for it to be the one page
    // on the site with a relative image.
    assert.strictEqual(
      og.image,
    ...
    console.log('PASS — no horizontal scroll at 400px width.');
  } finally {
    await browser.close();
    server.close();
  }
